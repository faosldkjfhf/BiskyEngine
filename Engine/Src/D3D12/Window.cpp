...
Window::Error Window::Init(IWindowCallbacks *windowCallbacks, UINT width, UINT height, const std::string &title)
{
  ...
  mWindow = CreateWindowExW(WS_EX_OVERLAPPEDWINDOW | WS_EX_APPWINDOW, (LPCWSTR)mWindowClass, stemp.c_str(),
                            WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, mWidth, mHeight, nullptr, nullptr,
...
void Window::SetFullscreen(bool enabled)
{
  ...
  SetWindowLongW(mWindow, GWL_STYLE, style);
  SetWindowLongW(mWindow, GWL_EXSTYLE, exStyle);
  ...
  if (enabled)
  {
    ...
    {
      SetWindowPos(mWindow, nullptr, info.rcMonitor.left, info.rcMonitor.top,
