...
UINT D3D12::ConstantBufferByteSize(UINT bytes)
{
  return (bytes + 255) & ~255;
}

ComPtr<ID3D12RootSignature> D3D12::CreateRootSignature(const D3D12::RootParameters &parameters,
...
ComPtr<ID3D12Resource> D3D12::CreateBuffer(ID3D12GraphicsCommandList10 *cmdList, ComPtr<ID3DBlob> &data,
                                           ComPtr<ID3D12Resource> &uploadBuffer)
{
  ...
  cmdList->ResourceBarrier(1, &D3D12::ResourceBarrier::Transition(buffer.Get(), D3D12_RESOURCE_STATE_COMMON,
                                                                  D3D12_RESOURCE_STATE_COPY_DEST));

  cmdList->CopyBufferRegion(buffer.Get(), 0, uploadBuffer.Get(), 0, data->GetBufferSize());

  cmdList->ResourceBarrier(1, &D3D12::ResourceBarrier::Transition(buffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
                                                                  D3D12_RESOURCE_STATE_GENERIC_READ));

  return buffer;
}

Ref<D3D12::Texture> D3D12::CreateTexture(ID3D12GraphicsCommandList10 *cmdList,
                                         const D3D12::Texture::ImageData &imageData)
{
